...
public class Utils {
	...
	public static List<Integer> uniqueRandoms(int n, int from, int to) {
		return uniqueRandoms(n, from, to, i -> true);
	}

	public static List<Integer> uniqueRandoms(int n, int from, int to, Predicate<Integer> filter) {
		if (from == to) return List.of();

		final List<Integer> set = new ArrayList<>();

		while (set.size() != n) {
			final int num = ThreadLocalRandom.current().nextInt(from, to);
			if (filter.test(num) && !set.contains(num)) set.add(num);
		}
